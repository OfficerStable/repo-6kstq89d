// English translations for LLM Graph Builder.
// Keys mirror the centralized dictionaries in utils/Constants.ts plus a `ui`
// namespace for inline component strings. English is the i18next fallback
// language, so any key missing from zh falls back to the value here.
const en = {
  language: {
    label: 'Language',
    switchTo: 'Switch to English',
    chinese: '中文',
    english: 'English',
  },
  buttonCaptions: {
    exploreGraphWithBloom: 'Explore Graph',
    showPreviewGraph: 'Preview Graph',
    deleteFiles: 'Delete Files',
    generateGraph: 'Generate Graph',
    dropzoneSpan: 'Documents, Images, Unstructured text',
    youtube: 'Youtube',
    gcs: 'GCS',
    amazon: 'Amazon S3',
    noLables: 'No Labels Found in the Database',
    dropYourCreds: 'Drop your neo4j credentials file here',
    analyze: 'Analyze text to extract graph schema',
    connect: 'Connect',
    disconnect: 'Disconnect',
    submit: 'Submit',
    connectToNeo4j: 'Connect to Neo4j',
    cancel: 'Cancel',
    details: 'Details',
    continueSettings: 'Continue',
    clearSettings: 'Clear Schema',
    ask: 'Ask',
    applyGraphSchema: 'Apply',
    provideAdditionalInstructions: 'Provide Processing Configuration for Entity Extractions',
    analyzeInstructions: 'Analyze Instructions',
    helpInstructions: 'Provide specific instructions for entity extraction, such as focusing on the key topics.',
    importDropzoneSpan: 'JSON Documents',
  },
  tooltips: {
    generateGraph: 'Generate graph from selected files',
    deleteFile: 'Select one or more files to delete',
    showGraph: 'Preview generated graph.',
    bloomGraph: 'Visualize the graph in Bloom',
    deleteSelectedFiles: 'File/Files to be deleted',
    previewGraphSelectedFiles: 'File/Files to Preview Graph',
    documentation: 'Documentation',
    github: 'GitHub Issues',
    theme: 'Light / Dark mode',
    settings: 'Entity Graph Extraction Settings',
    chat: 'Start a chat',
    sources: 'Upload files',
    deleteChat: 'Delete',
    maximise: 'Maximise',
    copy: 'Copy to Clipboard',
    copied: 'Copied',
    stopSpeaking: 'Stop Speaking',
    textTospeech: 'Text to Speech',
    createSchema: 'Define schema from text',
    useExistingSchema: 'Fetch schema from database',
    clearChat: 'Clear Chat History',
    continue: 'Continue',
    clearGraphSettings: 'Clear configured Graph Schema',
    applySettings: 'Apply Graph Schema',
    openChatPopout: 'Chat',
    downloadChat: 'Download Conversation',
    visualizeGraph: 'Visualize Graph Schema',
    additionalInstructions: 'Analyze instructions for schema',
    predinedSchema: 'Predefined Schema',
    dataImporterJson: 'Data Importer JSON',
  },
  appLabels: {
    ownSchema: 'Or Define your own Schema',
    predefinedSchema: 'Select a Pre-defined Schema',
    chunkingConfiguration: 'Select a Chunking Configuration',
    graphPatternTuple: 'Graph Pattern',
    selectedPatterns: 'Selected Patterns',
    dataImporterSchema: 'Schema from Data Importer',
  },
  llmDropdown: {
    disabledModels: 'Disabled models are available in the development version. Access more models in our ',
    devEnv: 'development environment',
  },
  connectionLabels: {
    notConnected: 'Not Connected',
    graphDataScience: 'Graph Data Science',
    graphDatabase: 'Graph Database',
  },
  graphLabels: {
    generateGraph: 'Generated Graph',
    inspectGeneratedGraphFrom: 'Inspect Generated Graph from',
    document: 'Document',
    chunk: 'Chunk',
    entities: 'Entities',
    resultOverview: 'Result Overview',
    totalNodes: 'Total Nodes',
    noEntities: 'No Entities Found',
    selectCheckbox: 'Select atleast one checkbox for graph view',
    totalRelationships: 'Total Relationships',
    docChunk: 'Document & Chunk',
    community: 'Communities',
    noNodesRels: 'No Nodes and No relationships',
    chunksInfo: 'We are visualizing 50 chunks at a time',
    renderSchemaGraph: 'Graph from Database Schema',
    generatedGraphFromUserSchema: 'Generated Graph from User Defined Schema',
  },
  chatMode: {
    unavailableChatMode: 'Chat mode is unavailable when files are selected',
    selected: 'Selected',
  },
  metrics: {
    faithfulness: 'Determines How accurately the answer reflects the provided information',
    answer_relevancy: "Determines How well the answer addresses the user's question.",
    rouge_score: 'Determines How much the generated answer matches the reference answer, word-for-word.',
    semantic_score: 'Determines How well the generated answer understands the meaning of the reference answer.',
    context_entity_recall: 'Determines the recall of entities present in both generated answer and retrieved contexts',
  },
  postProcessing: {
    materialize_text_chunk_similarities: `This option refines the connections between different pieces of information (chunks) within your
                knowledge graph. By leveraging a k-nearest neighbor algorithm with a similarity threshold (KNN_MIN_SCORE
                of 0.8), this process identifies and links chunks with high semantic similarity. This results in a more
                interconnected and insightful knowledge representation, enabling more accurate and relevant search
                results.`,
    enable_hybrid_search_and_fulltext_search_in_bloom: `This option optimizes search capabilities within your knowledge graph. It rebuilds the full-text index
                on database labels, ensuring faster and more efficient retrieval of information. This is particularly
                beneficial for large knowledge graphs, as it significantly speeds up keyword-based searches and improves
                overall query performance.`,
    materialize_entity_similarities: `Enhances entity analysis by generating numerical representations (embeddings) that capture their
                semantic meaning. This facilitates tasks like clustering similar entities, identifying duplicates, and
                performing similarity-based searches.`,
    enable_communities: 'Enable community creation across entities to use GraphRAG capabilities both local and global search.',
    graph_schema_consolidation:
      'This option uses the LLM for large graph schemas to consolidate many node labels and relationship types into fewer, more relevant ones and apply it to the extracted and existing graph',
  },
  ui: {
    back: 'Back',
    chatMode: 'Chat mode',
    login: 'Login',
    neo4jConnection: 'Neo4j Connection',
    connectionStatusConnected: 'Connected',
    connectionStatusNotConnected: 'Not Connected',
    noGraphSchemaConfigured: 'No Graph Schema configured',
    graphSettings: 'Graph Settings',
    llmModelForProcessing: 'LLM Model for Processing & Chat',
    dropzoneDefault: 'Drag & drop or select files',
    selectSource: 'Select Source',
    name: 'Name',
    status: 'Status',
    uploadStatus: 'Upload Status',
    size: 'Size (KB)',
    source: 'Source',
    type: 'Type',
    noDataAvailable: 'No data available',
    show: 'Show',
    showingResults: 'Showing {{start}} of {{total}} results',
    readOnlyMode: '(Read only Mode)',
    graphSchemaConfigured: 'Graph Schema configured',
    graphSchemaConfiguredCount: '({{labels}} Labels + {{rels}} Rel Types)',
    graphEnhancementSettings: 'Graph Enhancement Settings',
    enhanceGraphQuality: 'Enhance graph quality',
    pleaseLogInFirst: 'Please log in first',
    pleaseConnectToNeo4j: 'Please connect to Neo4j',
    pleaseLoginFirstToConnect: 'Please login first to connect',
    model: 'Model',
    nodes: 'Nodes',
    relations: 'Relations',
    tokenUsage: 'Token Usage',
    actions: 'Actions',
    embeddingModel: 'Embedding Model',
    availableInDevVersion: 'Available In Development Version',
    backendConnectionStatus: 'Backend connection status',
    connectToNeo4jToUpload: 'Connect to Neo4j to upload documents',
    sourceInfo: 'Source info',
    connectToNeo4jTitle: 'Connect to Neo4j',
    noNeo4jInstance: "Don't have a Neo4j instance? Start for free today",
    protocol: 'Protocol',
    uri: 'URI',
    databaseLabel: 'Database',
    usernameLabel: 'Username',
    passwordLabel: 'Password',
  },
  settings: {
    title: 'System Settings',
    tooltip: 'System Settings',
    tabLLM: 'LLM Models',
    tabSystem: 'System Options',
    llmIntro:
      'Configure LLM provider API keys and models. After saving, configured models appear in the "LLM Model for Processing & Chat" dropdown.',
    systemIntro: 'Optional advanced options. Leave a field blank to keep its default value.',
    addModel: 'Add Model',
    provider: 'Provider',
    displayName: 'Display Name',
    displayNameHint: 'Used to tell models apart in the list (e.g. My GPT-4o).',
    noModels: 'No models configured yet. Click "Add Model" to start.',
    modelKey: 'Model key',
    save: 'Save',
    saving: 'Saving…',
    saved: 'Settings saved. Refreshing…',
    saveFailed: 'Failed to save settings',
    loadFailed: 'Failed to load settings',
    remove: 'Remove',
    cancel: 'Cancel',
    requiredName: 'Please enter a display name for each model.',
    geminiHint: 'Gemini uses Google Cloud credentials (Vertex AI); configure ADC and PROJECT_ID in the environment.',
    backendUnavailable: 'Cannot reach the backend. Please make sure the app has finished starting up.',
    providerNames: {
      openai: 'OpenAI',
      openai_compatible: 'OpenAI-Compatible / Custom',
      anthropic: 'Anthropic (Claude)',
      groq: 'Groq',
      fireworks: 'Fireworks',
      azure: 'Azure OpenAI',
      bedrock: 'AWS Bedrock',
      ollama: 'Ollama (local)',
      diffbot: 'Diffbot',
      gemini: 'Google Gemini (Vertex AI)',
    },
    fieldNames: {
      model_name: 'Model name',
      api_key: 'API Key',
      api_endpoint: 'API Endpoint',
      base_url: 'Base URL',
      api_version: 'API Version',
      aws_access_key: 'AWS Access Key',
      aws_secret_key: 'AWS Secret Key',
      region_name: 'Region',
    },
    envNames: {
      EMBEDDING_MODEL: 'Embedding Model',
      EMBEDDING_PROVIDER: 'Embedding Provider',
      IS_EMBEDDING: 'Enable Embeddings (True/False)',
      OPENAI_API_KEY: 'OpenAI API Key (for embeddings)',
      KNN_MIN_SCORE: 'KNN Minimum Score',
      DUPLICATE_SCORE_VALUE: 'Duplicate Node Score',
      DUPLICATE_TEXT_DISTANCE: 'Duplicate Text Distance',
      GRAPH_CLEANUP_MODEL: 'Graph Cleanup Model',
      DEFAULT_DIFFBOT_CHAT_MODEL: 'Default Diffbot Chat Model',
      MAX_TOKEN_CHUNK_SIZE: 'Max Tokens per Chunk',
      UPDATE_GRAPH_CHUNKS_PROCESSED: 'Chunks Processed per Update',
      ENTITY_EMBEDDING: 'Entity Embedding (True/False)',
      EFFECTIVE_SEARCH_RATIO: 'Effective Search Ratio',
      NEO4J_URI: 'Neo4j URI (default)',
      NEO4J_USERNAME: 'Neo4j Username (default)',
      NEO4J_PASSWORD: 'Neo4j Password (default)',
      NEO4J_DATABASE: 'Neo4j Database (default)',
    },
  },
  tour: {
    loginHeader: 'Login with Neo4j',
    loginBody: 'Using Google Account or Email Address',
    connectHeader: 'Connect To Neo4j Database',
    connectBody: 'Fill out the neo4j credentials and click on connect',
    uploadHeader: 'Upload documents',
    uploadBody: 'Upload any unstructured files',
    llmHeader: 'Choose The Desired LLM',
    generateHeader: 'Start The Extraction Process',
    generateBody: 'Click On Generate Graph',
    visualizeHeader: 'Visualize The Knowledge Graph',
    visualizeBody: 'Select At Least One or More Completed Files From The Table For Visualization',
    chatHeader: 'Ask Questions Related To Documents',
  },
};

export default en;
