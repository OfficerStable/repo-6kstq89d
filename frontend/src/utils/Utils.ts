...
export const url = () => {
  ...
  // In the desktop (Electron) build the backend runs on a dynamically chosen
  // local port injected by the preload script; prefer it when present.
  const desktopBackendUrl = (window as unknown as { __BACKEND_URL__?: string }).__BACKEND_URL__;
